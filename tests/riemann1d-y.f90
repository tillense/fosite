...
!  CALL PrintDict(config)
     ...
     ALLOCATE(pvar0(Sim(ic)%Mesh%JMIN:Sim(ic)%Mesh%JMAX,1:3))
...
!           "filecycles" / 0, &
  ...
  SUBROUTINE InitData(Mesh,Physics,Timedisc,ic,pvar0) 
    ...
    REAL,DIMENSION(Mesh%JMIN:Mesh%JMAX,1:3),INTENT(OUT) :: pvar0
    !------------------------------------------------------------------------!
    ...
    CALL riemann(x0,GAMMA,rho_l,u_l,p_l,rho_r,u_r,p_r,TSIM,&
                 Mesh%bcenter(Mesh%IMIN,Mesh%JMIN:Mesh%JMAX,Mesh%KMIN,2),pvar0)
