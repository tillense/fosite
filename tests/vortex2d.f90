...
!             "datafile" / datafile)
  ...
  FUNCTION Run(Mesh,Physics,Timedisc) RESULT(sigma)
    ...
    REAL, DIMENSION(Mesh%IGMIN:Mesh%IGMAX,Mesh%JGMIN:Mesh%JGMAX,Mesh%KGMIN:Mesh%KGMAX,2) &
                      :: posvec,ephi,v0
    ...
    DO k=Mesh%KGMIN,Mesh%KGMAX
      DO j=Mesh%JGMIN,Mesh%JGMAX
         DO i=Mesh%IGMIN,Mesh%IGMAX
            ...
            domega = 0.5*VSTR/PI*EXP(0.5*(1.-(radius(i,j,k)/R0)**2))
            ...
            END SELECT
            Timedisc%pvar(i,j,k,Physics%XVELOCITY:Physics%YVELOCITY) = &
                    domega*radius(i,j,k)*ephi(i,j,k,1:2)
         END DO
      END DO
    END DO
