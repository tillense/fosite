...
CONTAINS
  ...
  SUBROUTINE InitBoundary_custom(this,Mesh,Physics,dir,config)
    ...
    CASE(WEST,EAST)
       ALLOCATE(this%cbtype(Mesh%JMIN:Mesh%JMAX,Mesh%KMIN:Mesh%KMAX,Physics%VNUM), &
            this%Rscale(Mesh%GNUM,Mesh%JMIN:Mesh%JMAX,Mesh%KMIN:Mesh%KMAX), &
            this%invRscale(Mesh%GNUM,Mesh%JMIN:Mesh%JMAX,Mesh%KMIN:Mesh%KMAX), &
            STAT=err)
    CASE(SOUTH,NORTH)
       ALLOCATE(this%cbtype(Mesh%IMIN:Mesh%IMAX,Mesh%KMIN:Mesh%KMAX,Physics%VNUM), &
            this%Rscale(Mesh%IMIN:Mesh%IMAX,Mesh%KMIN:Mesh%KMAX,Mesh%GNUM), &
            this%invRscale(Mesh%IMIN:Mesh%IMAX,Mesh%KGMIN:Mesh%KMAX,Mesh%GNUM), &
            STAT=err)
    CASE(BOTTOM,TOP)
       ALLOCATE(this%cbtype(Mesh%IMIN:Mesh%IMAX,Mesh%JMIN:Mesh%JMAX,Physics%VNUM), &
            this%Rscale(Mesh%IMIN:Mesh%IMAX,Mesh%JMIN:Mesh%JMAX,Mesh%GNUM), &
            this%invRscale(Mesh%IMIN:Mesh%IMAX,Mesh%KGMIN:Mesh%JMAX,Mesh%GNUM), &
            STAT=err)
    END SELECT
    IF (err.NE.0) THEN
       CALL this%Error("InitBoundary_custom", "Unable to allocate memory.")
